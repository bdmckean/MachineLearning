...
def split_cv(length, num_folds):
    ...
    # Each fold is the testing set in a split, and the remaining indices
    # are added to the corresponding training set.
    fold_set = np.array_split(indices, num_folds)
    for fold in range(num_folds):
        this_train = list(indices)
        for idx in fold_set[fold]:
            this_train.remove(idx)
        splits[fold] = (this_train, list(fold_set[fold]))
    return splits
...
def cv_performance(x, y, num_folds, k):
    ...
    for split in splits:
        ...
        split = SplitIndices(split[0], split[1])
        this_train_x = [ x[i] for i in split.train ]
        this_train_y = [ y[i] for i in split.train ]
        this_test_x = [ x[i] for i in split.test]
        this_test_y = [ y[i] for i in split.test]
        knn = Knearest(this_train_x, this_train_y, k)
        confusion = knn.confusion_matrix(this_test_x, this_test_y, debug=False)
